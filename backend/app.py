...
def score(request: ScoreRequest):
    ...
    return {"score": calculate_highest_grade(expected, actual) * 100}
...
def calculate_grade_for_groups(expected_movements, actual_movements):
    ...
    for i in range(len(expected_movements), len(actual_movements)):
        ...
        interpolated_data = fill_values(
            expected_movements, actual_movements[rang[0] : rang[1]]
        )
...
def calculate_highest_vectors(expected_movements, actual_movements):
    ...
    best_range_with_val = max(
        calculate_grade_for_groups(expected_movements, actual_movements).items(),
        key=lambda x: x[1],
    )
    best_range = best_range_with_val[0]
    desired_actual = actual_movements[best_range[0] : best_range[1]]
    curr_dict = calculate_vectors(
        expected_movements[0].landmarks, desired_actual[0].landmarks
    )
    for i in range(1, len(expected_movements)):
        dct = calculate_vectors(
            expected_movements[i].landmarks, desired_actual[i].landmarks
        )
